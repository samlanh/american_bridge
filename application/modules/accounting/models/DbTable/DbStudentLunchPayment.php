...
// 		}
    ...
    function getAllStudenTServicePayment($search){
    	...
    	$sql="select sp.id,
	    	...
	    	(select type from rms_student_paymentdetail where rms_student_paymentdetail.payment_id=sp.id limit 1)=5  ";
    	...
    	if(!empty($search['adv_search'])){
    		...
    		$s_where[] = " (select sv.stu_code from rms_service as sv where sv.stu_id=sp.student_id and  sv.type=5 ) LIKE '%{$s_search}%'";
	...
	function getAllNewStudentName($branch_id){
    	...
    	and s.stu_id NOT IN (select sv.stu_id from rms_service as sv where sv.status=1 and sv.type=5) and s.branch_id = $branch_id ";
	...
	function getAllOldStudent($branch_id){
    	...
    	where sv.stu_id = s.stu_id and sv.type=5 and sv.status=1 and s.is_subspend=0 and sv.branch_id = $branch_id ";
