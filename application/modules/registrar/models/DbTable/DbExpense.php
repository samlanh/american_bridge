...
function getAllExpense($search=null){
	...
	$sql=" SELECT id,invoice,title,(SELECT c.category_name FROM rms_category As c WHERE c.id=ln_income_expense.cat_id AND c.parent=0 LIMIT 1) as cat_name,
	(SELECT curr_nameen FROM `ln_currency` WHERE ln_currency.id =curr_type) AS currency_type, 
	total_amount,`desc`,for_date,status FROM $this->_name ";
